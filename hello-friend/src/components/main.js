import * as React from "react";
import './../style_sheets/main.css';
import {AppBar, Avatar, Grid} from "@mui/material";

const Main = () => {

    let emotes = ['😀', '😃', '😄', '😁', '😆', '😅', '😂', '🤣', '😇', '😉', '😊', '🙂', '🙃', '☺', '😋', '😌', '😍', '🥰', '😘', '😗', '😙', '😚', '🥲', '🤪', '😜', '😝', '😛', '🤑', '😎', '🤓', '🥸', '🧐', '🤠', '🥳', '🤡', '😏', '😶', '😑', '😒', '🙄', '🤨', '🤔', '🤫', '🤭', '🤥', '😳', '😞', '😟', '😤', '😠', '😡', '🤬', '😔', '😕', '🙁', '☹', '😬', '🥺', '😣', '😖', '😫', '😩', '🥱', '😪', '😮', '😱', '😨', '😰', '😥', '😓', '😯', '😦', '😧', '😢', '😭', '🤤', '🤩', '😵', '🥴', '😲', '🤯', '🤐', '😷', '🤕', '🤒', '🤮', '🤢', '🤧', '🥵', '🥶', '😴', '💤', '😈', '👿', '👹', '👺', '💩', '👻', '💀', '☠', '👽', '🤖', '🎃', '😺', '😸', '😹', '😻', '😼', '😽', '🙀', '😿', '😾', '👐', '🤲', '🙌', '👏', '🙏', '🤝', '👍', '👎', '👊', '✊', '🤛', '🤜', '🤞', '✌', '🤘', '🤟', '👌', '🤌', '🤏', '👈', '👉', '👆', '👇', '☝', '✋', '🤚', '🖐', '🖖', '👋', '🤙', '💪', '🦾', '🖕',  '✍', '🤳', '💅', '🦵', '🦿', '🦶', '👄', '🦷', '👅', '👂', '🦻', '👃', '👁', '👀', '🧠', '🫀', '🫁', '🦴', '👤', '👥', '🗣', '🫂', '👶', '👧', '🧒', '👦', '👩', '🧑', '👨', '👱', '👵', '🧓', '👴', '👲', '🙍', '💃', '🕺', '👫', '👭', '👬',  '💑', '❤', '🧡', '💛', '💚', '💙', '💜', '🤎', '🖤', '🤍', '💔', '❣', '💕', '💞', '💓', '💗', '💖', '💘', '💝', '💟', '🐶', '🐱', '🐭', '🐹', '🐰', '🐻', '🧸', '🐼',  '🐨', '🐯', '🦁', '🐮', '🐷', '🐽', '🐸', '🐵', '🙈', '🙉', '🙊', '🐒', '🦍', '🦧', '🐔', '🐧', '🐦', '🐤', '🐣', '🐥', '🐺', '🦊', '🦝', '🐗', '🐴', '🦓', '🦒', '🦌', '🦘', '🦥', '🦦', '🦫', '🦄', '🐝', '🐛', '🦋', '🐌', '🪲', '🐞', '🐜', '🦗', '🪳', '🕷', '🕸', '🦂', '🦟', '🪰', '🪱', '🦠', '🐢', '🐍', '🦎', '🐙', '🦑', '🦞', '🦀', '🦐', '🦪', '🐠', '🐟', '🐡', '🐬', '🦈', '🦭', '🐳', '🐋', '🐊', '🐆', '🐅', '🐃', '🐂', '🐄', '🦬', '🐪', '🐫', '🦙', '🐘', '🦏', '🦛', '🦣', '🐐', '🐏', '🐑', '🐎', '🐖', '🦇', '🐓', '🦃', '🕊', '🦅', '🦆', '🦢', '🦉', '🦩', '🦚', '🦜', '🦤', '🪶', '🐕', '🦮', '🐩', '🐈', '🐇', '🐀', '🐁', '🐿', '🦨', '🦡', '🦔', '🐾', '🐉', '🐲', '🦕', '🦖', '🌵', '🎄', '🌲', '🌳', '🌴', '🪴', '🌱', '🌿', '☘', '🍀', '🎍', '🎋', '🍃', '🍂', '🍁', '🌾', '🌺', '🌻', '🌹', '🥀', '🌷', '🌼', '🌸', '💐', '🍄', '🐚', '🌎', '🌍', '🌏', '🌕', '🌖', '🌗', '🌘', '🌑', '🌒', '🌓', '🌔', '🌙', '🌚', '🌝', '🌛', '🌜', '⭐', '🌟', '💫', '✨', '☄', '🪐', '🌞', '☀', '🌤', '⛅', '🌥', '🌦', '☁', '🌧', '⛈', '🌩', '⚡', '🔥', '💥', '❄', '🌨', '☃', '⛄', '🌬', '💨', '🌪', '🌫', '🌈', '☔', '💧', '💦', '🌊', '🍏', '🍎', '🍐', '🍊', '🍋', '🍌', '🍉', '🍇', '🍓', '🍈', '🍒', '🫐', '🍑', '🥭', '🍍', '🥥', '🥝', '🍅', '🥑', '🫒', '🍆', '🌶', '🫑', '🥒', '🥬', '🥦', '🧄', '🧅', '🌽', '🥕', '🥗', '🥔', '🍠', '🌰', '🥜', '🍯', '🍞', '🥐', '🥖', '🫓', '🥨', '🥯', '🥞', '🧇', '🧀', '🍗', '🍖', '🥩', '🍤', '🥚', '🍳', '🥓', '🍔', '🍟', '🌭', '🍕', '🍝', '🥪', '🌮', '🌯', '🫔', '🥙', '🧆', '🍜', '🥘', '🍲', '🫕', '🥫', '🧂', '🧈', '🍥', '🍣', '🍱', '🍛', '🍙', '🍚', '🍘', '🥟', '🍢', '🍡', '🍧', '🍨', '🍦', '🍰', '🎂', '🧁', '🥧', '🍮', '🍭', '🍬', '🍫', '🍿', '🍩', '🍪', '🥠', '🥮', '☕', '🍵', '🫖', '🥣', '🍼', '🥤', '🧋', '🧃', '🧉', '🥛', '🍺', '🍻', '🍷', '🥂', '🥃', '🍸', '🍹', '🍾', '🍶', '🧊', '🥄', '🍴', '🍽', '🥢', '🥡', '⚽', '🏀', '🏈', '⚾', '🥎', '🎾', '🏐', '🏉', '🎱', '🥏', '🪃', '🏓', '🏸', '🥅', '🏒', '🏑', '🏏', '🥍', '🥌', '⛳', '🏹', '🎣', '🤿', '🥊', '🥋', '⛸', '🎿', '🛷', '⛷', '🏂', '🎽', '🎖', '🏅', '🥇', '🥈', '🥉', '🏆', '🏵', '🎗', '🎫', '🎟', '🎪', '🎭', '🎨', '🎬', '🎤', '🎧', '🎼', '🎹', '🪗', '🥁', '🪘', '🎷', '🎺', '🎸', '🪕', '🎻', '🎲', '🧩', '♟', '🎯', '🎳', '🧝',  '🧟', '🧟', '🧟',  '🚗', '🚙', '🚕', '🛺', '🚌', '🚎', '🏎', '🚓', '🚑', '🚒', '🚐', '🛻', '🚚', '🚛', '🚜', '🏍', '🛵', '🚲', '🦼', '🦽', '🛴', '🛹', '🛼', '🚨', '🚔', '🚍', '🚘', '🚖', '🚡', '🚠', '🚟', '🚃', '🚋', '🚝', '🚄', '🚅', '🚈', '🚞', '🚂', '🚆', '🚇', '🚊', '🚉', '🚁', '🛩', '✈', '🛫', '🛬', '🪂', '💺', '🛰', '🚀', '🛸', '🛶', '⛵', '🛥', '🚤', '⛴', '🛳', '🚢', '⚓', '⛽', '🚧', '🚏', '🚦', '🚥', '🛑', '🎡', '🎢', '🎠', '🏗', '🌁', '🗼', '🏭', '⛲', '🎑', '⛰', '🏔', '🗻', '🌋', '🗾', '🏕', '⛺', '🏞', '🛣', '🛤', '🌅', '🌄', '🏜', '🏖', '🏝', '🌇', '🌆', '🏙', '🌃', '🌉', '🌌', '🌠', '🎇', '🎆', '🛖', '🏘', '🏰', '🏯', '🏟', '🗽', '🏠', '🏡', '🏚', '🏢', '🏬', '🏣', '🏤', '🏥', '🏦', '🏨', '🏪', '🏫', '🏩', '💒', '🏛', '⛪', '🕌', '🛕', '🕍', '🕋', '⛩', '⌚', '📱', '📲', '💻', '⌨', '🖥', '🖨', '🖱', '🖲', '🕹', '🗜', '💽', '💾', '💿', '📀', '📼', '📷', '📸', '📹', '🎥', '📽', '🎞', '📞', '☎', '📟', '📠', '📺', '📻', '🎙', '🎚', '🎛', '⏱', '⏲', '⏰', '🕰', '⏳', '⌛', '🧮', '📡', '🔋', '🔌', '💡', '🔦', '🕯', '🧯', '🗑', '🛢', '🛒', '💸', '💵', '💴', '💶', '💷', '💰', '🪙', '💳', '🧾', '💎', '⚖', '🦯', '🧰', '🔧', '🪛', '🔨', '⚒', '🛠', '⛏', '🪓', '🪚', '🔩', '⚙', '⛓', '🪝', '🪜', '🧱', '🪨', '🪵', '🔫', '🧨', '💣', '🔪', '🗡', '⚔', '🛡', '🚬', '⚰', '🪦', '⚱', '🏺', '🔮', '🪄', '📿', '🧿', '💈', '🧲', '⚗', '🧪', '🧫', '🧬', '🔭', '🔬', '🕳', '💊', '💉', '🩸', '🩹', '🩺', '🌡', '🏷', '🔖', '🚽', '🪠', '🚿', '🛁', '🛀', '🪥', '🪒', '🧴', '🧻', '🧼', '🧽', '🧹', '🧺', '🪣', '🔑', '🗝', '🪤', '🛋', '🪑', '🛌', '🛏', '🚪', '🪞', '🪟', '🧳', '🛎', '🖼', '🧭', '🗺', '⛱', '🗿', '🛍', '🎈', '🎏', '🎀', '🧧', '🎁', '🎊', '🎉', '🪅', '🪆', '🎎', '🎐', '🏮', '🪔', '✉', '📩', '📨', '📧', '💌', '📮', '📪', '📫', '📬', '📭', '📦', '📯', '📥', '📤', '📜', '📃', '📑', '📊', '📈', '📉', '📄', '📅', '📆', '🗓', '📇', '🗃', '🗳', '🗄', '📋', '🗒', '📁', '📂', '🗂', '🗞', '📰', '🪧', '📓', '📕', '📗', '📘', '📙', '📔', '📒', '📚', '📖', '🔗', '📎', '🖇', '✂', '📐', '📏', '📌', '📍', '🧷', '🪡', '🧵', '🧶', '🪢', '🔐', '🔒', '🔓', '🔏', '🖊', '🖋', '✒', '📝', '✏', '🖍', '🖌', '🔍', '🔎', '👚', '👕', '🥼', '🦺', '🧥', '👖', '👔', '👗', '👘', '🥻', '🩱', '👙', '🩲', '🩳', '💄', '💋', '👣', '🧦', '🩴', '👠', '👡', '👢', '🥿', '👞', '👟', '🩰', '🥾', '🧢', '👒', '🎩', '🎓', '👑', '⛑', '🪖', '🎒', '👝', '👛', '👜', '💼', '👓', '🕶', '🥽', '🧣', '🧤', '💍', '🌂', '☂'];
    var min = 1;
    var max = 1000;

    const [emot, setEmot] = React.useState(String);
    const [time, setTime] = React.useState(() => {
        setInterval(() => {
            var rand =  min + (Math.random() * (max-min));
            setTime(new Date().toString().substring(15, 24) + " ");
            setEmot(emotes[Math.round(rand)]);
        }, 500);
    });

    return (
        <Grid className={"main-container"} container direction={"column"} justifyContent={"center"} alignItems={"center"}>

            <Grid className={"icon-container"}>
                <Avatar className={"me"} alt="Barbary Théo" src="/images/me.jpeg" sx={{ width: 80, height: 80 }}/>
            </Grid>

            <Grid container direction={"row"} justifyContent={"center"} >
                <Grid item id={"time"} className={"time"}>
                    { time }
                </Grid>
                <Grid item id={"emot"} className={"emot"}>
                    { emot }
                </Grid>
            </Grid>

            <Grid item className={"hi"}>
                Hello friends
            </Grid>

            <Grid className={"barbary-container"}>
                <img className={"barbary"} src={"/images/barbary.png"} alt={"Barbary"}/>
            </Grid>
        </Grid>
    )

}

export default Main;